...
(function wirePrimaryUi() {
  ...
  async function mmOpenManualOrderPopup(opts) {
    ...
    step2.appendChild(row("Quote snapshot (AX-side anchor; 0 = disabled)", floatInput("moQs", gqs0, "e.g. 726.48")));
    step2.appendChild(row("Pricer snapshot (theo-side anchor; 0 = disabled)", floatInput("moPs", gps0, "e.g. 7280.7")));
  ...
  // Four-level Master Template grid.
  //
  // Each enabled row produces the same payload as the existing manual-order
  // popup and is submitted through the existing:
  //
  //   POST /api/desk/place_order_async
  //   POST /api/desk/place_order_async_status
  //
  async function mmOpenMasterTemplateGrid() {
    var state = window.__mmDeskLastState || {};
    var templatesDoc = {};
    var MASTER_TEMPLATE_KEY =
      "__MASTER_GRID__";

    var MASTER_TEMPLATE_TYPE =
      "master_grid";

    var selectedMasterTemplateName = "";

    try {
      var templatesResponse = await api("/api/desk/templates", {}, 10000);
      if (templatesResponse && templatesResponse.ok) {
        templatesDoc =
          templatesResponse.templates &&
          typeof templatesResponse.templates === "object"
            ? templatesResponse.templates
            : {};
      }
    } catch (e) {
      console.warn("Master Template: template load failed", e);
      // The grid can still be used without loading saved templates.
    }

    var instruments =
      Array.isArray(state.ax_grid_symbols) &&
      state.ax_grid_symbols.length
        ? state.ax_grid_symbols.slice()
        : Array.isArray(state.ax_book_symbols)
          ? state.ax_book_symbols.slice()
          : [];

    instruments = instruments.filter(function(symbol) {
      return (
        String(symbol || "")
          .trim()
          .toUpperCase() !== "__MASTER_GRID__"
      );
    });

    // Include products present only in ordinary templates.json
    // records, but exclude the reserved master-template key.
    Object.keys(templatesDoc).forEach(function(ax) {
      var normalizedAx = String(ax || "")
        .trim()
        .toUpperCase();

      if (
        normalizedAx === "__MASTER_GRID__"
      ) {
        return;
      }

      var exists = instruments.some(function(item) {
        return (
          String(item || "")
            .trim()
            .toUpperCase() === normalizedAx
        );
      });

      if (!exists) {
        instruments.push(String(ax));
      }
    });

    if (!instruments.length) {
      alert(
        "No AX instruments are available. Wait for GET /instruments to finish and try again."
      );
      return;
    }
    ...
    async function setOrdersFreeze(active) {
      try {
        var response = await api(
          "/api/desk/orders_config_freeze",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        console.warn("Master Template: orders freeze", active, e);
        return false;
      }
    }

    var freezeOk = await setOrdersFreeze(true);
    if (!freezeOk) {
      alert("Could not freeze orders.json reconciliation. Grid was not opened.");
      return;
    }
    ...
    title.style.cssText =
      ...
      "white-space:nowrap;";
    title.textContent =
      "Master Template — Four Quote Levels";

    var masterTemplateControls =
      document.createElement("div");

    masterTemplateControls.style.cssText =
      "display:flex;" +
      "align-items:center;" +
    ...
    var masterTemplateLabel =
      document.createElement("label");

    masterTemplateLabel.textContent =
      "Master template";

    masterTemplateLabel.style.cssText =
      "font-size:12px;" +
      "color:#aebdd1;" +
      "white-space:nowrap;";

    var masterTemplateSelect =
      document.createElement("select");

    masterTemplateSelect.style.cssText =
      "min-width:250px;" +
      "padding:7px;" +
      "background:#0b1220;" +
      "color:#e6edf3;" +
    ...
    var masterTemplateDeleteButton =
      document.createElement("button");

    masterTemplateDeleteButton.type = "button";
    masterTemplateDeleteButton.className =
      "btn btn-danger";
    masterTemplateDeleteButton.textContent =
      "Delete";

    masterTemplateDeleteButton.style.cssText =
      "background:#b91c1c;" +
      "border-color:#7f1d1d;" +
      "color:#fff;";

    masterTemplateDeleteButton.disabled = true;

    masterTemplateControls.appendChild(
      masterTemplateLabel
    );
    ...
    hint.className = "muted";
    hint.style.cssText =
      "font-size:12px;margin:0 0 12px 0;line-height:1.5";
    hint.textContent =
      "Quote all levels at the same time.";
    card.appendChild(hint);
    ...
    function makeInput(type, placeholder, width) {
    ...
    function findInstrumentLeg(ax) {
      var rows = Array.isArray(state.mm_instruments)
        ? state.mm_instruments
        : [];

      var upper = String(ax || "").toUpperCase();
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        var symbol = String(
          leg.symbol || leg.ax_symbol || ""
        ).toUpperCase();

        if (symbol === upper) return leg;
      }

      return null;
    }

    // Fill Snapshot Code Start
        /**
     * Convert the form's theo_source value to the key used by
     * /api/state.feed_health.
     *
     * Form values:
     *   neon_fix
     *   hyperliquid
     ...
     *   mettraders
     */
    function masterSnapshotFeedHealthKey(theoSource) {
      var source = String(theoSource || "")
        .trim()
        .toLowerCase();

      if (source === "neon_fix" || source === "neon") {
        return "neon";
      }

      if (source === "hyperliquid" || source === "hl") {
        return "hyperliquid";
      }

      if (source === "mettraders" || source === "mettrader") {
        return "mettraders";
      }
    ...
    /**
     * Return a case-insensitive book from an object such as:
     *
     *   state.ax_all_books
     *   state.ref_all_books
     */
    function masterSnapshotFindBook(books, symbol) {
      if (
        !books ||
        typeof books !== "object" ||
        symbol == null
      ...
      var wantedUpper = wanted.toUpperCase();
      var keys = Object.keys(books);

      for (var i = 0; i < keys.length; i++) {
        if (
          String(keys[i]).trim().toUpperCase() ===
          wantedUpper
        ) {
          return books[keys[i]];
        }
    ...
    /**
     * Extract a price from a book level.
     *
     * Supported level forms include:
     *
     *   [price, quantity]
     *   { price: 1.23, quantity: 100 }
     *   { p: 1.23, q: 100 }
     *   { px: 1.23, qty: 100 }
     */
    function masterSnapshotLevelPrice(level) {
      if (Array.isArray(level)) {
        var arrayPrice = Number(level[0]);

        return Number.isFinite(arrayPrice)
          ? arrayPrice
          : null;
      }
      ...
      if (level && typeof level === "object") {
        ...
        var objectPrice = Number(rawPrice);

        return Number.isFinite(objectPrice)
          ? objectPrice
          : null;
      }

      var numericPrice = Number(level);
    ...
    /**
     * Calculate the midpoint from the best bid and best ask.
     *
     * Returns null if:
     *   - the book is absent;
     *   - either side is empty;
     *   - either top price is invalid;
     *   - bid/ask prices are non-positive;
     *   - the ask is below the bid.
     */
    function masterSnapshotBookMid(book) {
      if (
        !book ||
        typeof book !== "object" ||
        book._err
      ...
      var bids = Array.isArray(book.bids)
        ? book.bids
        : [];

      var asks = Array.isArray(book.asks)
        ? book.asks
        : [];
      ...
      var bestBid = masterSnapshotLevelPrice(bids[0]);
      var bestAsk = masterSnapshotLevelPrice(asks[0]);
      ...
      var midpoint = (bestBid + bestAsk) / 2;
    ...
    /**
     * Check the C++-published feed-health snapshot.
     *
     * The button must remain disabled when:
     *   - no recognized source was chosen;
     *   - feed_health is missing;
     *   - the selected feed reports up=false;
     *   - the whole health snapshot is stale.
     */
    function masterSnapshotFeedStatus(
      currentState,
      theoSource
    ) {
      var healthKey =
        masterSnapshotFeedHealthKey(theoSource);

      if (!healthKey) {
        return {
          ok: false,
          reason: "Choose a supported Theo Source."
        };
      }

      var health =
        currentState &&
        currentState.feed_health &&
        typeof currentState.feed_health === "object"
          ? currentState.feed_health
          : null;
      ...
      if (!health) {
        return {
          ok: false,
          reason: "Feed-health data is unavailable."
        };
      }
      ...
      if (health.stale) {
        return {
          ok: false,
          reason:
            "Feed-health data is stale. Wait for trading_client."
        };
      }

      var selectedHealth = health[healthKey];
      ...
      if (
        ...
        var feedError =
          ...
          selectedHealth.last_error
            ? String(selectedHealth.last_error)
            : "";
    ...
    /**
     * Resolve the current AX and theo midpoints for a grid row.
     *
     * The latest window.__mmDeskLastState is read every time. This is
     * important because the popup may remain open while /api/state
     * continues refreshing in the background.
     */
        function masterSnapshotResolve(model) {
      var currentState =
        window.__mmDeskLastState || {};

      var axSymbol = String(
        model.axSelect.value || ""
      ).trim();

      var theoSource = String(
        model.theoSource.value || ""
      )
        .trim()
        .toLowerCase();

      var theoSymbol = String(
        model.theoSymbol.value || ""
      ).trim();

      if (!axSymbol) {
        return {
          ok: false,
          reason: "Choose an AX Product."
        };
      }
      ...
      if (!theoSymbol) {
        return {
          ok: false,
          reason: "Enter a Theo Symbol."
        };
      }

      var feedStatus =
        masterSnapshotFeedStatus(
          currentState,
          theoSource
        );
      ...
      var axBooks =
        currentState.ax_all_books &&
        typeof currentState.ax_all_books === "object"
          ? currentState.ax_all_books
          : {};

      var referenceBooks =
        currentState.ref_all_books &&
        typeof currentState.ref_all_books === "object"
          ? currentState.ref_all_books
          : {};

      // Use the codebase's existing case-insensitive lookup helper.
      var axBook = bookLookup(
        axBooks,
        axSymbol
      );
      ...
      if (
        !axBook &&
        String(currentState.ax_symbol || "")
          .trim()
          .toUpperCase() === axSymbol.toUpperCase()
      ) {
        axBook = currentState.book_ax || null;
      }

      var theoBook = bookLookup(
        referenceBooks,
        theoSymbol
      );
      ...
      if (
        !theoBook &&
        String(currentState.ref_symbol || "")
          .trim()
          .toUpperCase() === theoSymbol.toUpperCase()
      ) {
        theoBook =
          currentState.book_reference || null;
      }
      ...
      if (!axBook) {
        return {
          ok: false,
          reason:
            "No AX book found for " +
            axSymbol +
            ". Available AX books: " +
            (
              Object.keys(axBooks).length
                ? Object.keys(axBooks).join(", ")
                : "none"
            )
        };
      }
      ...
      if (!theoBook) {
        return {
          ...
          reason:
            ...
            " book found for " +
            theoSymbol +
            ". Available theo books: " +
            (
              Object.keys(referenceBooks).length
                ? Object.keys(referenceBooks).join(", ")
                : "none"
            )
        };
      }

      // Reuse the exact parser already used by the frontend's book
      // metrics display. It returns {mid, spread, spreadBps, ...}.
      var axMetrics = bookMetricsFromLevels(
        axBook.bids || [],
        axBook.asks || []
      );

      var theoMetrics = bookMetricsFromLevels(
        theoBook.bids || [],
        theoBook.asks || []
      );
      ...
      if (
        !axMetrics ||
        !Number.isFinite(Number(axMetrics.mid)) ||
        Number(axMetrics.mid) <= 0
      ) {
        return {
          ok: false,
          reason:
            "AX book for " +
            axSymbol +
            " does not have a valid bid/ask midpoint."
        };
      }

      if (
        !theoMetrics ||
        !Number.isFinite(Number(theoMetrics.mid)) ||
        Number(theoMetrics.mid) <= 0
      ) {
        return {
          ok: false,
          reason:
      ...
      return {
        ok: true,
        reason: "",
        quoteMid: Number(axMetrics.mid),
        pricerMid: Number(theoMetrics.mid),
        axSymbol: axSymbol,
        theoSource: theoSource,
        theoSymbol: theoSymbol
      };
    }

    /**
     * Format snapshots without unnecessarily rounding instruments that
     * require additional decimal places.
     *
     * Number(...) removes trailing zeroes while the maximum precision
     * protects against long floating-point strings.
     */
    function masterSnapshotFormat(value) {
      ...
      return String(
        Number(numericValue.toFixed(10))
      );
    }

    /**
     * Enable/disable one row's Fill Snapshot button.
     *
     * The reason is placed in the button tooltip.
     */
        function updateMasterSnapshotButton(model) {
      if (
        !model ||
      ...
      var button = model.snapshotButton;

      function paintDisabled(reason) {
        button.disabled = true;
        button.title =
          reason || "Snapshot is unavailable.";

        // Clearly disabled: dark grey and faded.
      ...
      function paintArmed(resolved) {
        button.disabled = false;

        button.title =
          "Ready. Fill Quote Snapshot from " +
          resolved.axSymbol +
          " midpoint and Pricer Snapshot from " +
          resolved.theoSource +
          " " +
          resolved.theoSymbol +
          " midpoint.";
      ...
      if (!model.enabled.checked) {
        paintDisabled("Enable this level first.");
        return;
      }
    ...
    /**
     * Refresh every row's button state.
     */
    function updateAllMasterSnapshotButtons() {
      rowModels.forEach(function(model) {
        updateMasterSnapshotButton(model);
      });
    }

        function applyInstrumentDefaults(model) {
      var leg = findInstrumentLeg(
        model.axSelect.value
      );
      ...
      var source = String(
        leg.theo_source || "neon_fix"
      )
        .trim()
        .toLowerCase();

      model.theoSource.value = source;

      if (
        source === "neon_fix" ||
        source === "neon"
      ) {
        model.theoSymbol.value = String(
          leg.reference_fix_symbol ||
          leg.theo_venue_symbol ||
          leg.theo_symbol ||
          ""
        ).trim();
      } else {
    ...
    function createGridRow(level) {
      ...
      var axSelect = makeSelect("145px");

      instruments.forEach(function(symbol) {
        appendOption(axSelect, String(symbol), String(symbol));
      });

      var theoSource = makeSelect("125px");
      ["mettraders", "hyperliquid", "neon_fix"].forEach(
        function(source) {
          appendOption(theoSource, source, source);
        }
      );

      var theoSymbol = makeInput("text", "Reference", "130px");
      var maxPosition = makeInput("number", "required", "110px");
      var width = makeInput("number", "required", "90px");
      var orderSize = makeInput("number", "required", "100px");
      var adjustPosition = makeInput("number", "required", "110px");
      var adjustTicks = makeInput("number", "required", "90px");
      var minDrift = makeInput("number", "required", "90px");
      var maxReload = makeInput("number", "0 = unlimited", "105px");
      var quoteSnapshot = makeInput("number", "0 = disabled", "120px");
      var pricerSnapshot = makeInput("number", "0 = disabled", "120px");
      var slope = makeInput("number", "1", "75px");

      maxReload.value = "0";
      quoteSnapshot.value = "0";
      pricerSnapshot.value = "0";
      slope.value = "1";
      ...
      snapshotButton.textContent = "Fill Snapshot";
      snapshotButton.disabled = true;
      snapshotButton.style.cssText =
        "min-width:115px;" +
        "white-space:nowrap;";
      snapshotButton.title =
        "Choose an active Theo Source and wait for AX/theo books.";
      ...
      var model = {
        ...
        adjustTicks: adjustTicks,
        minDrift: minDrift,
        maxReload: maxReload,
        quoteSnapshot: quoteSnapshot,
        pricerSnapshot: pricerSnapshot,
        slope: slope,
      ...
      snapshotButton.onclick = function() {
        console.log(
          "[Fill Snapshot] clicked",
          {
            level: model.level,
            ax: model.axSelect.value,
            theoSource: model.theoSource.value,
            theoSymbol: model.theoSymbol.value,
            latestState: window.__mmDeskLastState
          }
        );
        ...
        // These are ordinary editable inputs. Filling them does not lock
        // the values; the operator can still change either snapshot.
        model.quoteSnapshot.value =
          masterSnapshotFormat(
            resolved.quoteMid
          );

        model.pricerSnapshot.value =
          masterSnapshotFormat(
            resolved.pricerMid
          );

        model.status.style.color = "#86efac";
      ...
      axSelect.onchange = function() {
        applyInstrumentDefaults(model);
        updateMasterSnapshotButton(model);
      };

      theoSource.onchange = function() {
        // Changing the source can change both feed availability and the
        // meaning of the Theo Symbol.
        updateMasterSnapshotButton(model);
      };

      theoSymbol.oninput = function() {
        updateMasterSnapshotButton(model);
      };

      theoSymbol.onchange = function() {
        updateMasterSnapshotButton(model);
      };

      enabled.onchange = function() {
        tr.style.opacity =
          enabled.checked ? "1" : ".45";

        status.textContent =
          enabled.checked ? "Ready" : "Disabled";
    ...
    function getMasterTemplates() {
      var rows =
        templatesDoc &&
        Array.isArray(
          templatesDoc[MASTER_TEMPLATE_KEY]
        )
          ? templatesDoc[MASTER_TEMPLATE_KEY]
          : [];
      ...
      return rows.filter(function(template) {
        return (
          ...
          Array.isArray(template.params.rows)
        );
      });
    }

    function refreshMasterTemplateDropdown(
      preferredName
    ) {
      masterTemplateSelect.innerHTML = "";
      ...
      masterTemplateSelect.appendChild(
        placeholder
      );
      ...
      var wanted = String(
        preferredName ||
        selectedMasterTemplateName ||
        ""
      );

      if (wanted) {
        masterTemplateSelect.value = wanted;
      }

      selectedMasterTemplateName =
        masterTemplateSelect.value || "";

      masterTemplateDeleteButton.disabled =
        !selectedMasterTemplateName;
    }

    function findMasterTemplateByName(name) {
    ...
    // Populate the header dropdown from templates.json.
    refreshMasterTemplateDropdown("");

    var masterSnapshotAvailabilityTimer =
      setInterval(function() {
        if (!overlay.isConnected) {
          clearInterval(
            masterSnapshotAvailabilityTimer
          );
          return;
        }

        updateAllMasterSnapshotButtons();
      }, 750);

    var resultSummary = document.createElement("p");
    resultSummary.className = "muted";
    ...
    function clearMasterGridRow(model) {
      model.enabled.checked = false;
      model.row.style.opacity = ".45";

      model.maxPosition.value = "";
      model.width.value = "";
      model.orderSize.value = "";
      model.adjustPosition.value = "";
      model.adjustTicks.value = "";
      model.minDrift.value = "";
      model.maxReload.value = "0";

      model.quoteSnapshot.value = "0";
      model.pricerSnapshot.value = "0";
      model.slope.value = "1";

      model.status.style.color = "";
      model.status.textContent = "Disabled";

      updateMasterSnapshotButton(model);
    }

    function loadMasterGridRow(
      model,
      savedRow
    ) {
      if (
      ...
      if (axSymbol) {
        var optionExists =
          Array.from(
            model.axSelect.options
          ).some(function(option) {
            return (
              String(
                option.value
              ).toUpperCase() ===
              axSymbol.toUpperCase()
            );
          });
        ...
        if (!optionExists) {
          ...
          newOption.value = axSymbol;
          newOption.textContent = axSymbol;

          model.axSelect.appendChild(
            newOption
          );
        }

        model.axSelect.value = axSymbol;
      }

      setMasterField(
        model.theoSource,
        savedRow.theo_source
      );

      setMasterField(
        model.theoSymbol,
        savedRow.theo_venue_symbol ||
          savedRow.reference_fix_symbol
      );

      setMasterField(
        model.maxPosition,
        savedRow.instrument_max_position
      );
      ...
      setMasterField(
        ...
        savedRow.adjust_position
      );

      setMasterField(
        model.adjustTicks,
        savedRow.adjust_ticks
      );
      ...
      setMasterField(
        model.maxReload,
        savedRow.max_reload_cycles != null
          ? savedRow.max_reload_cycles
          : 0
      );

      setMasterField(
        model.quoteSnapshot,
        savedRow.quote_snapshot != null
          ? savedRow.quote_snapshot
          : 0
      );

      setMasterField(
        model.pricerSnapshot,
        savedRow.pricer_snapshot != null
          ? savedRow.pricer_snapshot
          : 0
      );
    ...
    function loadSelectedMasterTemplate() {
      var name = String(
        masterTemplateSelect.value || ""
      );

      selectedMasterTemplateName = name;
      ...
      resultSummary.style.color = "#86efac";
      resultSummary.textContent =
        "Loaded master template: " + name;
    }

    masterTemplateSelect.onchange =
      function() {
        loadSelectedMasterTemplate();
      };

    function readMasterOptionalInteger(
      field,
      fallback
    ) {
      var raw = String(
    ...
    function readMasterOptionalNumber(
      field,
      fallback
    ) {
      var raw = String(
    ...
    function buildMasterTemplateParams() {
      return {
        template_type:
          MASTER_TEMPLATE_TYPE,

        rows: rowModels.map(function(model) {
          return {
            enabled:
              !!model.enabled.checked,
    ...
    async function suggestMasterTemplateName() {
      var templates = getMasterTemplates();
      var names = {};

      templates.forEach(function(template) {
        if (template && template.name) {
          names[String(template.name)] = true;
        }
      });
      ...
      while (
        names["Master_Template_" + index]
      ) {
        index++;
      }

      return "Master_Template_" + index;
    }

    async function saveMasterTemplate(mode) {
      var currentName =
        selectedMasterTemplateName ||
        String(
          masterTemplateSelect.value || ""
      ...
      if (
        ...
        var suggested =
          await suggestMasterTemplateName();
        ...
        if (!name) {
          alert(
            "Master template name is required."
          );
      ...
      var params =
        buildMasterTemplateParams();

      var enabledCount =
        params.rows.filter(function(row) {
          return row.enabled;
        }).length;

      if (!enabledCount) {
        alert(
          "Enable at least one grid level before saving."
        );
      ...
      if (
        ...
        body.old_name = currentName;
      }

      resultSummary.style.color = "";
      resultSummary.textContent =
        mode === "update"
        ...
        if (!response || !response.ok) {
          throw new Error(
            response && response.error
              ? response.error
              : "Master template save failed"
          );
        }

        templatesDoc =
          response.templates &&
          typeof response.templates === "object"
            ? response.templates
            : templatesDoc;
    ...
    masterTemplateDeleteButton.onclick =
      async function() {
        var name = String(
          masterTemplateSelect.value || ""
        );
        ...
        // ============================================================
    // POINT 11 — BOTTOM ACTION BUTTONS
    // ============================================================
    ...
    var saveAsMasterButton =
      document.createElement("button");

    saveAsMasterButton.type = "button";
    saveAsMasterButton.className = "btn";
    saveAsMasterButton.textContent =
      "Save As New Template";

    var updateMasterButton =
      document.createElement("button");

    updateMasterButton.type = "button";
    updateMasterButton.className = "btn";
    updateMasterButton.textContent =
    ...
    function updateMasterSaveButtonStates() {
      var hasSelectedTemplate =
        !!String(
          masterTemplateSelect.value || ""
        );

      updateMasterButton.disabled =
        !hasSelectedTemplate;
    ...
    saveAsMasterButton.onclick =
      function() {
        void saveMasterTemplate("create");
      };

    updateMasterButton.onclick =
      function() {
        if (
          !masterTemplateSelect.value
        ) {
          alert(
            "Select a master template first."
          );

          return;
        }

        void saveMasterTemplate("update");
      };

    masterTemplateSelect.addEventListener(
      "change",
      function() {
        updateMasterSaveButtonStates();
      }
    );
    ...
    function readRequiredInteger(field, label, minimum, level) {
      var raw = String(field.value || "").trim();

      if (raw === "") {
        throw new Error(
          "Level " + level + ": " + label + " is required."
        );
      }
    ...
    function readOptionalNumber(field, fallback, label, level) {
      var raw = String(field.value || "").trim();
      if (raw === "") return fallback;
      ...
      if (!Number.isFinite(value)) {
        throw new Error(
          "Level " + level + ": " + label + " must be a number."
        );
      }
    ...
    function buildPayload(model) {
      ...
      var ax = String(model.axSelect.value || "").trim();
      var theoSource = String(model.theoSource.value || "").trim();
      var theoSymbol = String(model.theoSymbol.value || "").trim();

      if (!ax) {
        throw new Error("Level " + level + ": AX Product is required.");
      }

      if (!theoSource) {
        throw new Error("Level " + level + ": Theo Source is required.");
      }

      return {
        ax_symbol: ax,
        theo_source: theoSource,
        theo_venue_symbol: theoSymbol,

        instrument_max_position: readRequiredInteger(
          model.maxPosition,
          "Max Position",
          1,
          level
        ...
        params: {
          ...
          order_size: readRequiredInteger(
            model.orderSize,
            "Order Size",
            1,
            level
          ...
          adjust_position: readRequiredInteger(
            model.adjustPosition,
            "Adjust Position",
            1,
            level
          ...
          adjust_ticks: readRequiredInteger(
            model.adjustTicks,
            "Adjust Ticks",
            1,
            level
          ...
          min_theo_move_ticks_to_requote: readRequiredInteger(
            model.minDrift,
            "Min Drift",
            1,
            level
          ...
          max_reload_cycles: readRequiredInteger(
            model.maxReload,
            "Max Reload",
            0,
            level
          ...
          quote_snapshot: readOptionalNumber(
            model.quoteSnapshot,
            0,
            "Quote Snapshot",
            level
          ...
          pricer_snapshot: readOptionalNumber(
            model.pricerSnapshot,
            0,
            "Pricer Snapshot",
            level
    ...
    async function pollJob(model, jobId) {
      while (true) {
        await sleep(500);
        ...
        if (!response.done) {
          ...
          model.status.textContent = "Submitting...";
          continue;
        }
    ...
    async function submitOneRow(item) {
      ...
      if (
        ...
        throw new Error(
          startResponse && startResponse.error
            ? startResponse.error
            : "Could not start placement"
        );
      }

      var result = await pollJob(
        model,
        String(startResponse.job_id)
      );
    ...
    closeButton.onclick = async function() {
      if (submitInProgress) {
        alert("The grid is still submitting. Wait for all levels to finish.");
        return;
      }
    ...
    submitButton.onclick = async function() {
      ...
      var enabledModels = rowModels.filter(function(model) {
        return model.enabled.checked;
      });

      if (!enabledModels.length) {
        alert("Enable at least one level.");
        return;
      }

      var submissions;
      try {
        // Build payloads in visual level order. Every row is validated before
        // the first live order is sent.
        submissions = enabledModels
          .slice()
          .sort(function(a, b) {
        ...
        // The backend does not allow max_position to decrease for subsequent
        // stacks on the same instrument. Enforce the same rule in the grid
        // before submitting any live orders.
        var previousMaxByInstrument = {};

        submissions.forEach(function(item) {
          var ax = String(
            item.payload.ax_symbol || ""
          ).trim().toUpperCase();

          var currentMax = Number(
            item.payload.instrument_max_position
          );

          var previous = previousMaxByInstrument[ax];

          if (
            previous != null &&
            currentMax < previous.maxPosition
          ) {
            throw new Error(
              "Level " +
              item.model.level +
              ...
              " is " +
              currentMax +
              ", which is lower than Level " +
              previous.level +
              " (" +
              previous.maxPosition +
              "). Enter Max Position from low to high."
            );
          }

          previousMaxByInstrument[ax] = {
      ...
      saveAsMasterButton.disabled = true;
      updateMasterButton.disabled = true;

      masterTemplateSelect.disabled = true;
      masterTemplateDeleteButton.disabled = true;

      resultSummary.style.color = "";
      resultSummary.textContent =
        "Submitting " + submissions.length + " levels...";

      submissions.forEach(function(item) {
        item.model.status.style.color = "#facc15";
        item.model.status.textContent = "Queued";
      });

      try {
        // Submit rows sequentially in visual level order.
        //
        // submitOneRow() does not return until the asynchronous backend job
        // reaches a terminal state, so Level 2 cannot start until Level 1 has
        // completely finished.
        var successes = 0;
        var failures = 0;
        var stoppedAtLevel = null;
        ...
        for (var index = 0; index < submissions.length; index++) {
          ...
          try {
            await submitOneRow(item);
            successes++;
          } catch (rowError) {
            failures++;
            stoppedAtLevel = model.level;

            model.status.style.color = "#fca5a5";
            model.status.textContent =
              "Failed: " +
              String(
                rowError &&
            ...
            // Stop after the first failure. This prevents later, larger levels
            // from being placed when an earlier level was rejected.
            for (
              var remainingIndex = index + 1;
              remainingIndex < submissions.length;
              remainingIndex++
            ) {
              var remainingModel =
                submissions[remainingIndex].model;

              remainingModel.status.style.color = "#94a3b8";
              remainingModel.status.textContent =
                "Not submitted — Level " +
                model.level +
                " failed";
            }
        ...
        resultSummary.style.color =
        failures > 0 ? "#fca5a5" : "#86efac";

        if (failures === 0) {
          resultSummary.textContent =
            "Finished: all " +
            successes +
            " levels were placed sequentially.";
        } else {
          resultSummary.textContent =
            "Stopped at Level " +
            stoppedAtLevel +
        ...
        if (failures === 0) {
          alert(
            "All " +
            successes +
            " enabled levels were placed successfully, line by line."
          );
        } else {
          alert(
            "Submission stopped at Level " +
            stoppedAtLevel +
            ". " +
            successes +
            " earlier level(s) were placed. Later levels were not submitted."
          );
        }
      } catch (e) {
        resultSummary.style.color = "#fca5a5";
        resultSummary.textContent = String(
          (e && e.message) || e
        );
        ...
        saveAsMasterButton.disabled = false;
        masterTemplateSelect.disabled = false;
  ...
  el = document.getElementById("btnMasterTemplate");
  if (el) {
    el.onclick = function() {
      void mmOpenMasterTemplateGrid();
    };
  }
